...
describe("report serializers", () => {
  ...
  it("escapes untrusted project and finding text in HTML", () => { const html = serializeHtmlReport(report); expect(html).toContain("&lt;safe project&gt;"); expect(html).not.toContain("<safe project>"); expect(html).toContain("AI-assisted code signals"); expect(html).toContain("72% likelihood"); expect(html).toContain("lines 1-2"); expect(html).toContain("tutorial-style phrasing"); expect(html).toContain("Score breakdown"); expect(html).toContain("Review history"); expect(html).toContain("Keep human review"); expect(html).toContain("<details>"); });
});
