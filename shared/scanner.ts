...
export type AiSignal = { file: string; score: number; confidence: "low" | "medium" | "high"; reasons: string[] };
...
export function scanFiles(projectName: string, files: ScanInputFile[]): ScanReport {
  ...
  for (const file of supported) {
    ...
    if (commentLines >= 3 && commentLines / Math.max(lines.length, 1) > 0.25) { points += 20; reasons.push("high explanatory-comment density"); }
    if (/\b(Here is|This function|This class|Generated by|AI-generated|implementation details|edge cases)\b/i.test(file.content)) { points += 25; reasons.push("tutorial-style or generation-marker phrasing"); }
    if (/\b(todo|fixme)\b/i.test(file.content) && /\b(robust|efficient|seamless|comprehensive)\b/i.test(file.content)) { points += 15; reasons.push("generic planning language paired with unresolved markers"); }
    const longLines = lines.filter((line) => line.trim().length > 120).length;
    if (longLines >= 2) { points += 10; reasons.push("repeated long, uniformly formatted lines"); }
    if (lines.length >= 12 && reasons.length >= 1) points += 10;
    const score = Math.min(100, points);
    if (score >= 35) aiSignals.push({ file: file.path, score, confidence: score >= 65 ? "high" : "medium", reasons });
  }
